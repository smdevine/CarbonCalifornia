...
# rm(test)
comp_SOM <- horizon_to_comp_layer(horizon_SPC = horizon_data_spc, depth = 75, comp_df = comp_data, top_depth = 50, layername='50_75')
comp_SOM$soil_depth <- soil_comp_depths[match(comp_SOM$cokey, names(soil_comp_depths))]
colnames(comp_SOM)
comp_SOM$kgOrg.m2_50_75cm[is.na(comp_SOM$SOM_50_75cm)] <- NA #because otherwise these become 0 due to SOC content calc function that removes NAs

length(unique(comp_SOM$cokey)) #29306, same as above
summary(comp_SOM$kgOrg.m2_50_75cm) #7407 NA
summary(comp_SOM$SOM_50_75cm)
sum(comp_SOM$kgOrg.m2_50_75cm==0, na.rm = TRUE) #48 still reported as zero
comp_SOM[which(comp_SOM$kgOrg.m2_50_75cm==0),]
head(comp_SOM)
lapply(comp_SOM, summary)
hist(comp_SOM$SOM_50_75cm)
unique(comp_SOM$compname[comp_SOM$kgOrg.m2_50_75cm==0])
...
sum(!is.na(comp_SOM$kgOrg.m2_50_75cm) & comp_SOM$soil_depth <= bottom_depth & comp_SOM$SOM_depth < layer_thickness) #1660 instances
head(comp_SOM[!is.na(comp_SOM$kgOrg.m2_50_75cm) & comp_SOM$soil_depth <= bottom_depth & comp_SOM$SOM_depth < layer_thickness,])

#number of instances where SOC stock is non-NA and SOM depth is greater than reported BD depth
sum(!is.na(comp_SOM$kgOrg.m2_50_75cm) & comp_SOM$SOM_depth > comp_SOM$BD_depth) #75 instances
...
write.csv(comp_SOM[!is.na(comp_SOM$kgOrg.m2_50_75cm) & comp_SOM$SOM_depth > comp_SOM$BD_depth,], file.path(SSURGOdir, 'intermediate results', subDir_name, paste0('SOMdepth_greater_BDdepth', fname_depth, Sys.Date(), '.csv')), row.names = FALSE)

cokeys_SOMdepth_greater_BD_depth <- unique(comp_SOM$cokey[!is.na(comp_SOM$kgOrg.m2_50_75cm) & comp_SOM$SOM_depth > comp_SOM$BD_depth])
...
comp_SOM$kgOrg.m2_50_75cm[!is.na(comp_SOM$kgOrg.m2_50_75cm) & comp_SOM$SOM_depth > comp_SOM$BD_depth] <- NA
sum(comp_SOM$kgOrg.m2_50_75cm==0, na.rm = TRUE) #now 0
comp_SOM[which(comp_SOM$kgOrg.m2_50_75cm==0),]

#convert these to NA
sum(is.na(comp_SOM$kgOrg.m2_50_75cm)) #7482
comp_SOM$kgOrg.m2_50_75cm[which(comp_SOM$kgOrg.m2_50_75cm==0 & !is.na(comp_SOM$SOM_50_75cm))] <- NA
sum(is.na(comp_SOM$kgOrg.m2_50_75cm))

#how many instances where SOM data depth exceeds rock frag depth
sum(!is.na(comp_SOM$kgOrg.m2_50_75cm) & comp_SOM$SOM_depth > comp_SOM$Rock_Frag_depth) #0
comp_SOM[!is.na(comp_SOM$kgOrg.m2_50_75cm) & comp_SOM$SOM_depth > comp_SOM$Rock_Frag_depth,]
horizon_data_majcomps[horizon_data_majcomps$cokey %in% comp_SOM$cokey[!is.na(comp_SOM$kgOrg.m2_50_75cm) & comp_SOM$SOM_depth > comp_SOM$Rock_Frag_depth],]

#convert SOC content to NA when SOM data depth exceeds rock frag depth
sum(is.na(comp_SOM$kgOrg.m2_50_75cm))
comp_SOM$kgOrg.m2_50_75cm[!is.na(comp_SOM$kgOrg.m2_50_75cm) & comp_SOM$SOM_depth > comp_SOM$Rock_Frag_depth] <- NA
sum(!is.na(comp_SOM$kgOrg.m2_50_75cm) & comp_SOM$SOM_depth > comp_SOM$Rock_Frag_depth) #0 now
sum(is.na(comp_SOM$kgOrg.m2_50_75cm))

#how many instances of shallow soils with complete SOM data
sum(!is.na(comp_SOM$kgOrg.m2_50_75cm) & comp_SOM$soil_depth <= bottom_depth & comp_SOM$SOM_depth == layer_thickness) #82

#how many instances of shallow soils with "incomplete" SOM data
sum(!is.na(comp_SOM$kgOrg.m2_50_75cm) & comp_SOM$soil_depth <= bottom_depth & comp_SOM$SOM_depth < layer_thickness) #1649, but not all of these are of real concern, as deduced below

#investigate these cokeys further 
problematic_cokeys <- comp_SOM[!is.na(comp_SOM$kgOrg.m2_50_75cm) & comp_SOM$soil_depth <= bottom_depth & comp_SOM$SOM_depth < layer_thickness,]
dim(problematic_cokeys) #1649 rows
length(unique(problematic_cokeys$cokey)) #1649 
...
length(unique(problematic_cokeys_H123$cokey)) #569
length(unique(problematic_horizons_H123$cokey)) #569
...
dim(problematic_cokeys_H123) #569 that should be dealt with
length(unique(problematic_cokeys_H123$mukey)) #affecting 553 mukeys
sum(mu_data$hectares[mu_data$mukey %in% problematic_cokeys_H123$mukey]) #1,332,170 ha
...
dim(problematic_cokeys_noH123) #1080 rows
sum(mu_data$hectares[mu_data$mukey %in% problematic_cokeys_noH123$mukey]) #affecting  2,005,229 hectares
...
horizon_data_majcomps[horizon_data_majcomps$cokey==22523454,] #Cd horizon
horizon_data_majcomps[horizon_data_majcomps$cokey==22735839,] #Cd horizon
horizon_data_majcomps[horizon_data_majcomps$cokey==22805457,] #Cd horizon
horizon_data_majcomps[horizon_data_majcomps$cokey==23471909,] #Cd horizon
...
comp_SOM$kgOrg.m2_50_75cm[comp_SOM$cokey %in% problematic_cokeys_noH123$cokey] <- NA
sum(is.na(comp_SOM$kgOrg.m2_50_75cm)) #7488

sum(is.na(comp_SOM$SOM_50_75cm)) #7407
comp_SOM$SOM_50_75cm[comp_SOM$cokey %in% problematic_cokeys_noH123$cokey] <- NA
sum(is.na(comp_SOM$SOM_50_75cm)) #7413
...
sum(!is.na(comp_SOM$kgOrg.m2_50_75cm) & (comp_SOM$soil_depth > bottom_depth) & (comp_SOM$SOM_depth < layer_thickness)) #1842 soils
problematic_cokeys_v2 <- comp_SOM[!is.na(comp_SOM$kgOrg.m2_50_75cm) & (comp_SOM$soil_depth > bottom_depth) & (comp_SOM$SOM_depth < layer_thickness),]
dim(problematic_cokeys_v2) #1842 rows
length(unique(problematic_cokeys_v2$cokey)) #1842
...
length(unique(problematic_cokeys_H123_v2$cokey)) #1592
length(unique(problematic_horizons_H123_v2$cokey)) #1592
...
dim(problematic_cokeys_H123_v2) #1592 that should be dealt with
length(unique(problematic_cokeys_H123_v2$mukey)) #affecting 1514 mukeys
sum(mu_data$hectares[mu_data$mukey %in% problematic_cokeys_H123_v2$mukey]) #2,922,773 ha
...
dim(problematic_cokeys_noH123_v2) #250 rows
sum(problematic_cokeys_H123_v2$cokey %in% problematic_cokeys_noH123_v2$cokey)
sum(mu_data$hectares[mu_data$mukey %in% problematic_cokeys_noH123_v2$mukey]) #affecting  568780 hectares
...
sum(!is.na(problematic_cokeys_noH123_v2$SOM_50_75cm) & problematic_cokeys_noH123_v2$SOM_depth < layer_thickness) #250
sum(!is.na(problematic_cokeys_noH123_v2$kgOrg.m2_50_75cm) & problematic_cokeys_noH123_v2$SOM_depth < layer_thickness) #250
...
length(unique(problematic_horizons_H123_all$cokey)) #2161
table(problematic_horizons_H123_all$reskind_match) #FALSE 4591 TRUE 2084
head(problematic_horizons_H123_all)
length(unique(problematic_horizons_H123_all$cokey[problematic_horizons_H123_all$reskind_match==TRUE]))#2017
...
horizon_data_majcomps[horizon_data_majcomps$cokey==22485463,]
...
table(problematic_cokeys_H123_all$SOM_QC) #FALSE 198 TRUE 1963 
head(problematic_cokeys_H123_all[problematic_cokeys_H123_all$SOM_QC==FALSE,])
problematic_cokeys_H123_all[problematic_cokeys_H123_all$cokey==22521892,]
problematic_horizons_H123_all[problematic_horizons_H123_all$cokey==22521892,]
reskind_df[reskind_df$cokey==22521892,]
horizon_data_majcomps[horizon_data_majcomps$cokey==22521892,]
...
sum(is.na(comp_SOM$kgOrg.m2_50_75cm)) #7488
comp_SOM$kgOrg.m2_50_75cm[comp_SOM$cokey %in% problematic_cokeys_H123_SOM_NA$cokey] <- NA
sum(is.na(comp_SOM$kgOrg.m2_50_75cm)) #7686

sum(is.na(comp_SOM$SOM_50_75cm)) #7413
comp_SOM$SOM_50_75cm[comp_SOM$cokey %in% problematic_cokeys_H123_SOM_NA$cokey] <- NA
sum(is.na(comp_SOM$SOM_50_75cm)) #7611

#dealing with problematic_cokeys_noH123_v2 issue, where SOM_depth is less than layer thickness but soil depth is 
dim(problematic_cokeys_noH123_v2) #250
summary(problematic_cokeys_noH123_v2$SOM_50_75cm)
summary(problematic_cokeys_noH123_v2$kgOrg.m2_50_75cm)
...
#make SOC and SOM NA when the SOM data was less than soil depth
sum(is.na(comp_SOM$kgOrg.m2_50_75cm)) #7686
comp_SOM$kgOrg.m2_50_75cm[comp_SOM$cokey %in% problematic_cokeys_noH123_v2$cokey] <- NA
sum(is.na(comp_SOM$kgOrg.m2_50_75cm)) #7936

sum(is.na(comp_SOM$SOM_50_75cm)) #7611
comp_SOM$SOM_50_75cm[comp_SOM$cokey %in% problematic_cokeys_noH123_v2$cokey] <- NA
sum(is.na(comp_SOM$SOM_50_75cm)) #7861
...
comppct_by_mukey_om_data <- data.frame(mukey=row.names(tapply(comp_SOM$comppct[!is.na(comp_SOM$SOM_50_75cm)], comp_SOM$mukey[!is.na(comp_SOM$SOM_50_75cm)], sum)), comppct_tot=as.numeric(tapply(comp_SOM$comppct[!is.na(comp_SOM$SOM_50_75cm)], comp_SOM$mukey[!is.na(comp_SOM$SOM_50_75cm)], sum)), stringsAsFactors = FALSE)
summary(comppct_by_mukey_om_data$comppct_tot)
sum(comppct_by_mukey_om_data$comppct_tot < 70) #3582 mukeys less than 70
sum(comppct_by_mukey_om_data$comppct_tot==15) #155 have only 15% of components with data
sum(comppct_by_mukey_om_data$comppct_tot < 15) #0, because major component defined as >15% of mapunit

comppct_by_mukey_soc_data <- data.frame(mukey=row.names(tapply(comp_SOM$comppct[!is.na(comp_SOM$kgOrg.m2_50_75cm)], comp_SOM$mukey[!is.na(comp_SOM$kgOrg.m2_50_75cm)], sum)), comppct_tot=as.numeric(tapply(comp_SOM$comppct[!is.na(comp_SOM$kgOrg.m2_50_75cm)], comp_SOM$mukey[!is.na(comp_SOM$kgOrg.m2_50_75cm)], sum)), stringsAsFactors = FALSE)
summary(comppct_by_mukey_soc_data$comppct_tot)

CA_50_75cm_muagg <- MUAggregate_wrapper(df1=comp_SOM, varnames = c('SOM_50_75cm', 'kgOrg.m2_50_75cm'))
CA_50_75cm_muagg$compct_SOM_50_75cm <- comppct_by_mukey_om_data$comppct_tot[match(CA_50_75cm_muagg$mukey, comppct_by_mukey_om_data$mukey)]
CA_50_75cm_muagg$compct_SOCkg_50_75cm <- comppct_by_mukey_soc_data$comppct_tot[match(CA_50_75cm_muagg$mukey, comppct_by_mukey_soc_data$mukey)]
CA_50_75cm_muagg$hectares <- mu_data$hectares[match(CA_50_75cm_muagg$mukey, mu_data$mukey)]
summary(CA_50_75cm_muagg$hectares)
sum(CA_50_75cm_muagg$hectares[which(CA_50_75cm_muagg$compct_SOM==15.00)]) #417680.8
sum(CA_50_75cm_muagg$hectares) #32,827,071 total hectares
sum(CA_50_75cm_muagg$hectares[!is.na(CA_50_75cm_muagg$SOM_50_75cm)]) #25,976,273
sum(mu_data$hectares) #41,477,744

#add result to file with all mukeys
mu_data$SOM_50_75cm <- CA_50_75cm_muagg$SOM_50_75cm[match(mu_data$mukey, CA_50_75cm_muagg$mukey)]
mu_data$compct_SOM_50_75cm <- CA_50_75cm_muagg$compct_SOM_50_75cm[match(mu_data$mukey, CA_50_75cm_muagg$mukey)]
mu_data$kgSOC.m2_50_75cm <- CA_50_75cm_muagg$kgOrg.m2_50_75cm[match(mu_data$mukey, CA_50_75cm_muagg$mukey)]
mu_data$compct_SOCkg_50_75cm <- CA_50_75cm_muagg$compct_SOCkg_50_75cm[match(mu_data$mukey, CA_50_75cm_muagg$mukey)]

sum(mu_data$hectares[!is.na(mu_data$SOM_50_75cm)])/sum(mu_data$hectares) #62.6% of AOI with SOM summaries
length(mu_data$mukey[!is.na(mu_data$SOM_50_75cm)])/length(mu_data$mukey) #81.8% of mukeys
...
# mu_data$SOM_50_75cm[mu_data$mukey==455490] #0.2960937, correct
# 
...
# 2.5*50/95+1.5*45/95
# mu_data$SOM_50_75cm[mu_data$mukey==457102] #correct

#identify some breakpoints
SOM_wtd_data <- unlist(mapply(function(x, y) {rep(x, times=round(y/10, 0))}, x=mu_data$SOM_50_75cm, y=mu_data$hectares))
...
SOC_wtd_data <- unlist(mapply(function(x, y) {rep(x, times=round(y/10, 0))}, x=mu_data$kgSOC.m2_50_75cm, y=mu_data$hectares))
...
sum(mu_data$hectares[which(mu_data$SOM_50_75cm < 1.1911765)]) #11,866,719 hectares
11866719/sum(mu_data$hectares[!is.na(mu_data$SOM_50_75cm)])
...
mu_diffs <- mu_data_30cm[!is.na(mu_data_30cm$SOM_30cm) & is.na(mu_data$SOM_50_75cm),]
dim(mu_diffs) #1557
head(mu_diffs, 20)
