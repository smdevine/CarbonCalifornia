...
length(unique(comp_SOM_100cm$cokey)) #29306, same as above
summary(comp_SOM_100cm$kgOrg.m2_100cm) #1711 NA
...
sum(!is.na(comp_SOM_100cm$kgOrg.m2_100cm) & comp_SOM_100cm$soil_depth <= 100 & comp_SOM_100cm$SOM_depth < comp_SOM_100cm$soil_depth) #4527 instances
...
sum(!is.na(comp_SOM_100cm$kgOrg.m2_100cm) & comp_SOM_100cm$SOM_depth > comp_SOM_100cm$BD_depth) #269 instances
write.csv(comp_SOM_100cm[!is.na(comp_SOM_100cm$kgOrg.m2_100cm) & comp_SOM_100cm$SOM_depth > comp_SOM_100cm$BD_depth,], file.path(SSURGOdir, 'intermediate results', '0_100cm_aggregation', paste0('SOMdepth_greater_BDdepth_100cm_', Sys.Date(), '.csv')), row.names = FALSE)
...
write.csv(test, file.path(SSURGOdir, 'intermediate results', '0_100cm_aggregation', paste0('SOMdepth_greater_BDdepth_missingBDhorizons_', Sys.Date(), '.csv')), row.names = FALSE)
rm(test)
...
sum(!is.na(comp_SOM_100cm$kgOrg.m2_100cm) & comp_SOM_100cm$soil_depth <= 100 & comp_SOM_100cm$SOM_depth == comp_SOM_100cm$soil_depth) #4573

#how many instances of shallow soils with "incomplete" SOM data
sum(!is.na(comp_SOM_100cm$kgOrg.m2_100cm) & comp_SOM_100cm$soil_depth <= 100 & comp_SOM_100cm$SOM_depth < comp_SOM_100cm$soil_depth) #4522
...
dim(problematic_cokeys) #4853 rows
length(unique(problematic_cokeys$cokey))
write.csv(problematic_cokeys, file.path(SSURGOdir, 'intermediate results', '0_100cm_aggregation', paste0('problematic_cokeys_100cm_', Sys.Date(), '.csv')), row.names = FALSE)
problematic_horizons <- horizon_data_majcomps[horizon_data_majcomps$cokey %in% problematic_cokeys$cokey,]
write.csv(problematic_horizons, file.path(SSURGOdir, 'intermediate results', '0_100cm_aggregation', paste0('problematic_horizons_100cm_', Sys.Date(), '.csv')), row.names = FALSE)
...
write.csv(problematic_cokeys_H123, file.path(SSURGOdir, 'intermediate results', '0_100cm_aggregation', paste0('problematic_cokeys_H123_100cm_', Sys.Date(), '.csv')), row.names = FALSE)
problematic_horizons_H123 <- problematic_horizons[problematic_horizons$cokey %in% problematic_cokeys_H123$cokey, ]
length(unique(problematic_cokeys_H123$cokey)) #4180
length(unique(problematic_horizons_H123$cokey)) #4180
write.csv(problematic_horizons_H123, file.path(SSURGOdir, 'intermediate results', '0_100cm_aggregation', paste0('problematic_horizons_H123_100cm_', Sys.Date(), '.csv')), row.names = FALSE)
dim(problematic_cokeys_H123) #4180 that should be dealt with
length(unique(problematic_cokeys_H123$mukey)) #affecting 3387 mukeys
sum(mu_data$hectares[mu_data$mukey %in% problematic_cokeys_H123$mukey]) #8,076,418 ha
...
write.csv(problematic_cokeys_noH123, file.path(SSURGOdir, 'intermediate results', '0_100cm_aggregation', paste0('problematic_cokeys_noH123_100cm_', Sys.Date(), '.csv')), row.names = FALSE)
dim(problematic_cokeys_noH123) #342 rows
sum(mu_data$hectares[mu_data$mukey %in% problematic_cokeys_noH123$mukey]) #affecting 548974.6 hectares

problematic_horizons_noH123 <- problematic_horizons[problematic_horizons$cokey %in% problematic_cokeys_noH123$cokey,]
write.csv(problematic_horizons_noH123, file.path(SSURGOdir, 'intermediate results', '0_100cm_aggregation', paste0('problematic_horizons_noH123_100cm_', Sys.Date(), '.csv')), row.names = FALSE)
...
sum(is.na(comp_SOM_100cm$kgOrg.m2_100cm)) #1983
comp_SOM_100cm$kgOrg.m2_100cm[comp_SOM_100cm$cokey %in% problematic_cokeys_noH123$cokey] <- NA
sum(is.na(comp_SOM_100cm$kgOrg.m2_100cm)) #2325

sum(is.na(comp_SOM_100cm$SOM_100cm)) #1711
comp_SOM_100cm$SOM_100cm[comp_SOM_100cm$cokey %in% problematic_cokeys_noH123$cokey] <- NA
sum(is.na(comp_SOM_100cm$SOM_100cm)) #2053

#look at cases where soil depth > 100 cm but SOM data depth < 100
#will use also component restriction table to make decisions about these cases
sum(!is.na(comp_SOM_100cm$kgOrg.m2_100cm) & comp_SOM_100cm$soil_depth > 100 & comp_SOM_100cm$SOM_depth < 100) #4008 soils
problematic_cokeys_v2 <- comp_SOM_100cm[!is.na(comp_SOM_100cm$kgOrg.m2_100cm) & comp_SOM_100cm$soil_depth > 100 & comp_SOM_100cm$SOM_depth < 100,]
dim(problematic_cokeys_v2) #4008 rows
length(unique(problematic_cokeys_v2$cokey)) #4008
...
write.csv(problematic_cokeys_H123_v2, file.path(SSURGOdir, 'intermediate results', '0_100cm_aggregation', paste0('problematic_cokeys_H123_v2_100cm_', Sys.Date(), '.csv')), row.names = FALSE)
problematic_horizons_H123_v2 <- problematic_horizons_v2[problematic_horizons_v2$cokey %in% problematic_cokeys_H123_v2$cokey, ]
length(unique(problematic_cokeys_H123_v2$cokey)) #3055
length(unique(problematic_horizons_H123_v2$cokey)) #3055
unique(problematic_horizons_H123_v2$hzname)
write.csv(problematic_horizons_H123_v2, file.path(SSURGOdir, 'intermediate results', '0_100cm_aggregation', paste0('problematic_horizons_H123_v2_100cm_', Sys.Date(), '.csv')), row.names = FALSE)
dim(problematic_cokeys_H123_v2) #3055 that should be dealt with
length(unique(problematic_cokeys_H123_v2$mukey)) #affecting 2693 mukeys
sum(mu_data$hectares[mu_data$mukey %in% problematic_cokeys_H123_v2$mukey]) #4,583,426 ha

problematic_cokeys_noH123_v2 <- problematic_cokeys_v2[!(problematic_cokeys_v2$cokey %in% problematic_cokeys_H123_v2$cokey),]
dim(problematic_cokeys_noH123_v2) #953 rows
length(unique(problematic_cokeys_noH123_v2$cokey)) #953
write.csv(problematic_cokeys_noH123_v2, file.path(SSURGOdir, 'intermediate results', '0_100cm_aggregation', paste0('problematic_cokeys_noH123_v2_100cm_', Sys.Date(), '.csv')), row.names = FALSE)
sum(problematic_cokeys_H123_v2$cokey %in% problematic_cokeys_noH123_v2$cokey)
sum(mu_data$hectares[mu_data$mukey %in% problematic_cokeys_noH123_v2$mukey]) #affecting  1,606,988 ha hectares
...
write.csv(problematic_horizons_noH123_v2, file.path(SSURGOdir, 'intermediate results', '0_100cm_aggregation', paste0('problematic_horizons_noH123_v2_100cm_', Sys.Date(), '.csv')), row.names = FALSE)
...
write.csv(problematic_cokeys_H123_all, file.path(SSURGOdir, 'intermediate results', '0_100cm_aggregation', paste('problematic_cokeys_H123_all_', Sys.Date(), '.csv')), row.names = FALSE)
dim(problematic_cokeys_H123_all) #7235 rows
length(unique(problematic_cokeys_H123_all$cokey)) #7235
...
sum(test$resdept_r < 100, na.rm = TRUE) #19266
head(test)
unique(test$reskind)
...
write.csv(problematic_horizons_H123_all, file.path(SSURGOdir, 'intermediate results', '0_100cm_aggregation', paste0('problematic_horizons_H123_all_', Sys.Date(), '.csv')), row.names = FALSE)
...
length(unique(problematic_horizons_H123_all$cokey)) #7235
table(problematic_horizons_H123_all$reskind_match) #6923 matches
head(problematic_horizons_H123_all)
length(unique(problematic_horizons_H123_all$cokey[problematic_horizons_H123_all$reskind_match==TRUE]))#6711
problematic_horizons_H123_all[problematic_horizons_H123_all$cokey==22474987,]
reskind_df[reskind_df==22474987,]
comp_SOM_100cm[comp_SOM_100cm$cokey==21096542,]
sum(is.na(problematic_horizons_H123_all$reskind_match))
...
table(problematic_cokeys_H123_all$SOM_QC) #FALSE 905, TRUE 6330
head(problematic_cokeys_H123_all[problematic_cokeys_H123_all$SOM_QC==FALSE,])
problematic_cokeys_H123_all[problematic_cokeys_H123_all$cokey==22485039,]
problematic_horizons_H123_all[problematic_horizons_H123_all$cokey==22485039,]
reskind_df[reskind_df$cokey==22485039,]
write.csv(problematic_cokeys_H123_all, file.path(SSURGOdir, 'intermediate results', '0_100cm_aggregation', paste0('problematic_cokeys_H123_FINAL_100cm_', Sys.Date(), '.csv')), row.names = FALSE)
...
sum(is.na(comp_SOM_100cm$kgOrg.m2_100cm)) #3230

sum(is.na(comp_SOM_100cm$SOM_100cm)) #2053
comp_SOM_100cm$SOM_100cm[comp_SOM_100cm$cokey %in% problematic_cokeys_H123_SOM_NA$cokey] <- NA
sum(is.na(comp_SOM_100cm$SOM_100cm)) #2958

#plus issues with problematic_cokeys_noH123_v2
head(problematic_cokeys_noH123_v2)
dim(problematic_cokeys_noH123_v2) #953
sum(problematic_cokeys_noH123_v2$cokey %in% reskind_df$cokey) #560
...
#make SOC and SOM NA when the SOM data was less than 100, but soil depth when soil depth was > 100, as determined by conventional horizon nomenclature
sum(is.na(comp_SOM_100cm$kgOrg.m2_100cm)) #3230
comp_SOM_100cm$kgOrg.m2_100cm[comp_SOM_100cm$cokey %in% problematic_cokeys_noH123_v2$cokey] <- NA
sum(is.na(comp_SOM_100cm$kgOrg.m2_100cm)) #4183

sum(is.na(comp_SOM_100cm$SOM_100cm)) #2958
comp_SOM_100cm$SOM_100cm[comp_SOM_100cm$cokey %in% problematic_cokeys_noH123_v2$cokey] <- NA
sum(is.na(comp_SOM_100cm$SOM_100cm)) #3911
...
write.csv(comp_SOM_100cm, file.path(SSURGOdir, 'intermediate results', '0_100cm_aggregation', paste0('comp_SOM_100cm_', Sys.Date(), '.csv')), row.names = FALSE)
...
sum(comppct_by_mukey_om_data$comppct_tot < 70) #2481 mukeys less than 70
sum(comppct_by_mukey_om_data$comppct_tot==15) #50 have only 15% of components with data
...
sum(CA_100cm_muagg$hectares[which(CA_100cm_muagg$compct_SOM==15.00)]) #121179.8
sum(CA_100cm_muagg$hectares) #32,827,071 total hectares
sum(CA_100cm_muagg$hectares[!is.na(CA_100cm_muagg$SOM_100cm)]) #29,630,957
...
sum(mu_data$hectares[!is.na(mu_data$SOM_100cm)])/sum(mu_data$hectares) #71.4% of AOI with SOM summaries
length(mu_data$mukey[!is.na(mu_data$SOM_100cm)])/length(mu_data$mukey) #88.8% of mukeys
write.csv(mu_data, file.path(SSURGOdir, 'summaries', paste0('CA_mapunit_SOM_100cm_', Sys.Date(), '.csv')), row.names = FALSE)
...
mu_data_30cm <- read.csv(file.path(SSURGOdir, 'summaries', 'CA_mapunit_SOM_30cm_2022-10-11.csv'), stringsAsFactors = FALSE)
all(mu_data$mukey==mu_data_30cm$mukey)
mu_diffs <- mu_data_30cm[!is.na(mu_data_30cm$SOM_30cm) & is.na(mu_data$SOM_100cm),]
dim(mu_diffs) #874
head(mu_diffs, 20)
mu_data_30cm[mu_data_30cm$mukey==1015013,]
mu_data[mu_data$mukey==1015013,]
comp_SOM_100cm[comp_SOM_100cm$mukey==1015013,]
horizon_data_majcomps[horizon_data_majcomps$cokey==23488883,]
reskind_df[reskind_df$cokey==23488883,]
