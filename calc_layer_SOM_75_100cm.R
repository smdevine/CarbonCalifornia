...
# rm(test)
comp_SOM <- horizon_to_comp_layer(horizon_SPC = horizon_data_spc, depth = 100, comp_df = comp_data, top_depth = 75, layername='75_100')
comp_SOM$soil_depth <- soil_comp_depths[match(comp_SOM$cokey, names(soil_comp_depths))]
colnames(comp_SOM)
comp_SOM$kgOrg.m2_75_100cm[is.na(comp_SOM$SOM_75_100cm)] <- NA #because otherwise these become 0 due to SOC content calc function that removes NAs

length(unique(comp_SOM$cokey)) #29306, same as above
summary(comp_SOM$kgOrg.m2_75_100cm) #10829 NA
summary(comp_SOM$SOM_75_100cm)
sum(comp_SOM$kgOrg.m2_75_100cm==0, na.rm = TRUE) #59 still reported as zero
comp_SOM[which(comp_SOM$kgOrg.m2_75_100cm==0),]
head(comp_SOM)
lapply(comp_SOM, summary)
hist(comp_SOM$SOM_75_100cm)
unique(comp_SOM$compname[comp_SOM$kgOrg.m2_75_100cm==0])
...
sum(!is.na(comp_SOM$kgOrg.m2_75_100cm) & comp_SOM$soil_depth <= bottom_depth & comp_SOM$SOM_depth < layer_thickness) #1660 instances
head(comp_SOM[!is.na(comp_SOM$kgOrg.m2_75_100cm) & comp_SOM$soil_depth <= bottom_depth & comp_SOM$SOM_depth < layer_thickness,])

#number of instances where SOC stock is non-NA and SOM depth is greater than reported BD depth
sum(!is.na(comp_SOM$kgOrg.m2_75_100cm) & comp_SOM$SOM_depth > comp_SOM$BD_depth) #85 instances
...
write.csv(comp_SOM[!is.na(comp_SOM$kgOrg.m2_75_100cm) & comp_SOM$SOM_depth > comp_SOM$BD_depth,], file.path(SSURGOdir, 'intermediate results', subDir_name, paste0('SOMdepth_greater_BDdepth', fname_depth, Sys.Date(), '.csv')), row.names = FALSE)

cokeys_SOMdepth_greater_BD_depth <- unique(comp_SOM$cokey[!is.na(comp_SOM$kgOrg.m2_75_100cm) & comp_SOM$SOM_depth > comp_SOM$BD_depth])
...
comp_SOM$kgOrg.m2_75_100cm[!is.na(comp_SOM$kgOrg.m2_75_100cm) & comp_SOM$SOM_depth > comp_SOM$BD_depth] <- NA
sum(comp_SOM$kgOrg.m2_75_100cm==0, na.rm = TRUE) #now 0
comp_SOM[which(comp_SOM$kgOrg.m2_75_100cm==0),]

#convert these to NA
sum(is.na(comp_SOM$kgOrg.m2_75_100cm)) #10914
comp_SOM$kgOrg.m2_75_100cm[which(comp_SOM$kgOrg.m2_75_100cm==0 & !is.na(comp_SOM$SOM_75_100cm))] <- NA
sum(is.na(comp_SOM$kgOrg.m2_75_100cm))

#how many instances where SOM data depth exceeds rock frag depth
sum(!is.na(comp_SOM$kgOrg.m2_75_100cm) & comp_SOM$SOM_depth > comp_SOM$Rock_Frag_depth) #0
comp_SOM[!is.na(comp_SOM$kgOrg.m2_75_100cm) & comp_SOM$SOM_depth > comp_SOM$Rock_Frag_depth,]
horizon_data_majcomps[horizon_data_majcomps$cokey %in% comp_SOM$cokey[!is.na(comp_SOM$kgOrg.m2_75_100cm) & comp_SOM$SOM_depth > comp_SOM$Rock_Frag_depth],]

#convert SOC content to NA when SOM data depth exceeds rock frag depth
sum(is.na(comp_SOM$kgOrg.m2_75_100cm))
comp_SOM$kgOrg.m2_75_100cm[!is.na(comp_SOM$kgOrg.m2_75_100cm) & comp_SOM$SOM_depth > comp_SOM$Rock_Frag_depth] <- NA
sum(!is.na(comp_SOM$kgOrg.m2_75_100cm) & comp_SOM$SOM_depth > comp_SOM$Rock_Frag_depth) #0 now
sum(is.na(comp_SOM$kgOrg.m2_75_100cm))

#how many instances of shallow soils with complete SOM data
sum(!is.na(comp_SOM$kgOrg.m2_75_100cm) & comp_SOM$soil_depth <= bottom_depth & comp_SOM$SOM_depth == layer_thickness) #100

#how many instances of shallow soils with "incomplete" SOM data
sum(!is.na(comp_SOM$kgOrg.m2_75_100cm) & comp_SOM$soil_depth <= bottom_depth & comp_SOM$SOM_depth < layer_thickness) #1965, but not all of these are of real concern, as deduced below

#investigate these cokeys further 
problematic_cokeys <- comp_SOM[!is.na(comp_SOM$kgOrg.m2_75_100cm) & comp_SOM$soil_depth <= bottom_depth & comp_SOM$SOM_depth < layer_thickness,]
dim(problematic_cokeys) #1965 rows
length(unique(problematic_cokeys$cokey)) #1965 
...
length(unique(problematic_cokeys_H123$cokey)) #638
length(unique(problematic_horizons_H123$cokey)) #638
...
dim(problematic_cokeys_H123) #638 that should be dealt with
length(unique(problematic_cokeys_H123$mukey)) #affecting 626 mukeys
sum(mu_data$hectares[mu_data$mukey %in% problematic_cokeys_H123$mukey]) #1,360,251 ha
...
dim(problematic_cokeys_noH123) #1327 rows
sum(mu_data$hectares[mu_data$mukey %in% problematic_cokeys_noH123$mukey]) #affecting  2,192,980 hectares
...
horizon_data_majcomps[horizon_data_majcomps$cokey==22733935,] #erroneous soil depth calc
horizon_data_majcomps[horizon_data_majcomps$cokey==22936428,]
...
sum(is.na(comp_SOM$kgOrg.m2_75_100cm)) #10914
sum(comp_SOM$cokey %in% problematic_cokeys_noH123$cokey) #6
comp_SOM$kgOrg.m2_75_100cm[comp_SOM$cokey %in% problematic_cokeys_noH123$cokey] <- NA
sum(is.na(comp_SOM$kgOrg.m2_75_100cm)) #10923

sum(is.na(comp_SOM$SOM_75_100cm)) #10829
comp_SOM$SOM_75_100cm[comp_SOM$cokey %in% problematic_cokeys_noH123$cokey] <- NA
sum(is.na(comp_SOM$SOM_75_100cm)) #10838
...
sum(!is.na(comp_SOM$kgOrg.m2_75_100cm) & (comp_SOM$soil_depth > bottom_depth) & (comp_SOM$SOM_depth < layer_thickness)) #1785 soils
problematic_cokeys_v2 <- comp_SOM[!is.na(comp_SOM$kgOrg.m2_75_100cm) & (comp_SOM$soil_depth > bottom_depth) & (comp_SOM$SOM_depth < layer_thickness),]
dim(problematic_cokeys_v2) #1785 rows
length(unique(problematic_cokeys_v2$cokey)) #1785
...
length(unique(problematic_cokeys_H123_v2$cokey)) #1582
length(unique(problematic_horizons_H123_v2$cokey)) #1582
...
dim(problematic_cokeys_H123_v2) #1582 that should be dealt with
length(unique(problematic_cokeys_H123_v2$mukey)) #affecting 1472 mukeys
sum(mu_data$hectares[mu_data$mukey %in% problematic_cokeys_H123_v2$mukey]) #2,628,875 ha
...
dim(problematic_cokeys_noH123_v2) #203 rows
sum(problematic_cokeys_H123_v2$cokey %in% problematic_cokeys_noH123_v2$cokey)
sum(mu_data$hectares[mu_data$mukey %in% problematic_cokeys_noH123_v2$mukey]) #affecting  429399.5 hectares
...
sum(!is.na(problematic_cokeys_noH123_v2$SOM_75_100cm) & problematic_cokeys_noH123_v2$SOM_depth < layer_thickness) #203
sum(!is.na(problematic_cokeys_noH123_v2$kgOrg.m2_75_100cm) & problematic_cokeys_noH123_v2$SOM_depth < layer_thickness) #203
...
dim(problematic_cokeys_H123_all) #2220 rows
length(unique(problematic_cokeys_H123_all$cokey)) #2220
...
length(unique(problematic_horizons_H123_all$cokey)) #2220
table(problematic_horizons_H123_all$reskind_match) #FALSE 5404 TRUE 2105
head(problematic_horizons_H123_all)
length(unique(problematic_horizons_H123_all$cokey[problematic_horizons_H123_all$reskind_match==TRUE]))#2070
...
table(problematic_cokeys_H123_all$SOM_QC) #FALSE 228 TRUE 1992 
head(problematic_cokeys_H123_all[problematic_cokeys_H123_all$SOM_QC==FALSE,])
problematic_cokeys_H123_all[problematic_cokeys_H123_all$cokey==22538898,]
problematic_horizons_H123_all[problematic_horizons_H123_all$cokey==22538898,]
reskind_df[reskind_df$cokey==22538898,]
horizon_data_majcomps[horizon_data_majcomps$cokey==22538898,]
...
sum(is.na(comp_SOM$kgOrg.m2_75_100cm)) #10923
comp_SOM$kgOrg.m2_75_100cm[comp_SOM$cokey %in% problematic_cokeys_H123_SOM_NA$cokey] <- NA
sum(is.na(comp_SOM$kgOrg.m2_75_100cm)) #11151

sum(is.na(comp_SOM$SOM_75_100cm)) #10838
comp_SOM$SOM_75_100cm[comp_SOM$cokey %in% problematic_cokeys_H123_SOM_NA$cokey] <- NA
sum(is.na(comp_SOM$SOM_75_100cm)) #11066

#dealing with problematic_cokeys_noH123_v2 issue, where SOM_depth is less than layer thickness but soil depth is 
dim(problematic_cokeys_noH123_v2) #203
summary(problematic_cokeys_noH123_v2$SOM_75_100cm)
summary(problematic_cokeys_noH123_v2$kgOrg.m2_75_100cm)
summary(problematic_cokeys_noH123_v2$SOM_depth)
head(problematic_cokeys_noH123_v2)
problematic_horizons_noH123_v2[problematic_horizons_noH123_v2$cokey==22523515,]
reskind_df[reskind_df$cokey==22523515,]
problematic_horizons_noH123_v2[problematic_horizons_noH123_v2$cokey==22529879,]
reskind_df[reskind_df$cokey==22529879,]

#make SOC and SOM NA when the SOM data was less than soil depth
sum(is.na(comp_SOM$kgOrg.m2_75_100cm)) #11151
comp_SOM$kgOrg.m2_75_100cm[comp_SOM$cokey %in% problematic_cokeys_noH123_v2$cokey] <- NA
sum(is.na(comp_SOM$kgOrg.m2_75_100cm)) #11354

sum(is.na(comp_SOM$SOM_75_100cm)) #11066
comp_SOM$SOM_75_100cm[comp_SOM$cokey %in% problematic_cokeys_noH123_v2$cokey] <- NA
sum(is.na(comp_SOM$SOM_75_100cm)) #11269
...
comppct_by_mukey_om_data <- data.frame(mukey=row.names(tapply(comp_SOM$comppct[!is.na(comp_SOM$SOM_75_100cm)], comp_SOM$mukey[!is.na(comp_SOM$SOM_75_100cm)], sum)), comppct_tot=as.numeric(tapply(comp_SOM$comppct[!is.na(comp_SOM$SOM_75_100cm)], comp_SOM$mukey[!is.na(comp_SOM$SOM_75_100cm)], sum)), stringsAsFactors = FALSE)
summary(comppct_by_mukey_om_data$comppct_tot)
sum(comppct_by_mukey_om_data$comppct_tot < 70) #3560 mukeys less than 70
sum(comppct_by_mukey_om_data$comppct_tot==15) #159 have only 15% of components with data
sum(comppct_by_mukey_om_data$comppct_tot < 15) #0, because major component defined as >15% of mapunit

comppct_by_mukey_soc_data <- data.frame(mukey=row.names(tapply(comp_SOM$comppct[!is.na(comp_SOM$kgOrg.m2_75_100cm)], comp_SOM$mukey[!is.na(comp_SOM$kgOrg.m2_75_100cm)], sum)), comppct_tot=as.numeric(tapply(comp_SOM$comppct[!is.na(comp_SOM$kgOrg.m2_75_100cm)], comp_SOM$mukey[!is.na(comp_SOM$kgOrg.m2_75_100cm)], sum)), stringsAsFactors = FALSE)
summary(comppct_by_mukey_soc_data$comppct_tot)

CA_75_100cm_muagg <- MUAggregate_wrapper(df1=comp_SOM, varnames = c('SOM_75_100cm', 'kgOrg.m2_75_100cm'))
CA_75_100cm_muagg$compct_SOM_75_100cm <- comppct_by_mukey_om_data$comppct_tot[match(CA_75_100cm_muagg$mukey, comppct_by_mukey_om_data$mukey)]
CA_75_100cm_muagg$compct_SOCkg_75_100cm <- comppct_by_mukey_soc_data$comppct_tot[match(CA_75_100cm_muagg$mukey, comppct_by_mukey_soc_data$mukey)]
CA_75_100cm_muagg$hectares <- mu_data$hectares[match(CA_75_100cm_muagg$mukey, mu_data$mukey)]
summary(CA_75_100cm_muagg$hectares)
sum(CA_75_100cm_muagg$hectares[which(CA_75_100cm_muagg$compct_SOM==15.00)]) #349549.8
sum(CA_75_100cm_muagg$hectares) #32,827,071 total hectares
sum(CA_75_100cm_muagg$hectares[!is.na(CA_75_100cm_muagg$SOM_75_100cm)]) #21,955,647
sum(mu_data$hectares) #41,477,744

#add result to file with all mukeys
mu_data$SOM_75_100cm <- CA_75_100cm_muagg$SOM_75_100cm[match(mu_data$mukey, CA_75_100cm_muagg$mukey)]
mu_data$compct_SOM_75_100cm <- CA_75_100cm_muagg$compct_SOM_75_100cm[match(mu_data$mukey, CA_75_100cm_muagg$mukey)]
mu_data$kgSOC.m2_75_100cm <- CA_75_100cm_muagg$kgOrg.m2_75_100cm[match(mu_data$mukey, CA_75_100cm_muagg$mukey)]
mu_data$compct_SOCkg_75_100cm <- CA_75_100cm_muagg$compct_SOCkg_75_100cm[match(mu_data$mukey, CA_75_100cm_muagg$mukey)]

sum(mu_data$hectares[!is.na(mu_data$SOM_75_100cm)])/sum(mu_data$hectares) #52.9% of AOI with SOM summaries
length(mu_data$mukey[!is.na(mu_data$SOM_75_100cm)])/length(mu_data$mukey) #71.4% of mukeys
...
# mu_data$SOM_75_100cm[mu_data$mukey==455490] #0.2960937, correct
# 
...
# 2.5*50/95+1.5*45/95
# mu_data$SOM_75_100cm[mu_data$mukey==457102] #correct

#identify some breakpoints
SOM_wtd_data <- unlist(mapply(function(x, y) {rep(x, times=round(y/10, 0))}, x=mu_data$SOM_75_100cm, y=mu_data$hectares))
...
SOC_wtd_data <- unlist(mapply(function(x, y) {rep(x, times=round(y/10, 0))}, x=mu_data$kgSOC.m2_75_100cm, y=mu_data$hectares))
...
sum(mu_data$hectares[which(mu_data$SOM_75_100cm < 1.1911765)]) #11,866,719 hectares
11866719/sum(mu_data$hectares[!is.na(mu_data$SOM_75_100cm)])
...
mu_diffs <- mu_data_30cm[!is.na(mu_data_30cm$SOM_30cm) & is.na(mu_data$SOM_75_100cm),]
dim(mu_diffs) #1557
head(mu_diffs, 20)
...
#map unit
mu_data[mu_data$mukey==1015016,]
comp_SOM[comp_SOM$mukey==1015016,]
horizon_data_majcomps[horizon_data_majcomps$cokey==23488968,]
